...
def print_diagnostics(
    ...
    write_msg = "Will write to boot volume" if write else click.style("", fg="red")
...
def validate_config(vault: bool):
    ...
    drivers_list = list(map(lambda path: Path(path).name, get_user_drivers_list()))
    for driver in drivers_list:
        if driver not in list(
            ...
            click.echo(click.style(f"will add {driver} to config", fg="yellow"))
            config_plist["UEFI"]["Drivers"] = existing_drivers
            write_config_file(config_plist, USER_OPENCORE_DIR)
        else:
            click.echo(
                click.style(f"found driver {driver} in UEFI->Drivers", fg="green")
            )
...
def check_vault(signed: bool = True):
    ...
    files = list(map(lambda x: Path(dir) / "OC" / x, ["vault.plist", "vault.sig"]))
    for file in files:
        if not file.exists():
...
def download(url: str, dest_dir: str = ENV["DOWNLOADS_DIR"]):
    ...
    click.echo(click.style("Downloading " + url + " to " + str(dest_path), fg="green"))
    if dest_path.exists():
...
def start(
    ...
    click.echo(click.style(f"Building OpenCore {version}", fg="green", bold=True))
    if reset:
        cwd = os.getcwd()
        os.system(
            f"cd {HACKINTOSH_ROOT} && git add . && git reset HEAD --hard && cd {cwd}"
        )
        click.echo(click.style("Finished resetting EFI folder", fg="green", bold=True))
    ...
    if backup:
        ...
        run([f"/usr/local/bin/7z a {BACKUP_DESTINATION}.7z {BACKUP_DESTINATION}"])
    ...
    if not build:
        click.echo(
            click.style("Download finished. Exiting (launched with '--no-build')")
        )
        return 0
    if bless:
